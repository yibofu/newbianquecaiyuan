...
require_once(APP_PATH . "Common/Org/Pay/Driver/wapalipay/lib/alipay_core.function.php");
require_once(APP_PATH . "Common/Org/Pay/Driver/wapalipay/lib/alipay_rsa.function.php");
require_once(APP_PATH . "Common/Org/Pay/Driver/wapalipay/lib/alipay_md5.function.php");

class Wapalipay extends \Common\Org\Pay\Pay {
    ...
    public function buildRequestForm(\Common\Org\Pay\PayVo $vo) {
        ...
        //需http://格式的完整路径，不允许加?id=123这类自定义参数
        //操作中断返回地址
        $merchant_url = "http://shop.vmalltop.com/User/";
    ...
    public function verifyNotify($notify) {
        if ($_POST) {
            // D("test")->where("id=1")->save(array('test'=>"到类里面来测试了"));//测试可以删除
            //对notify_data解密
            ...
            //D("test")->where("id=1")->save(array('test'=>"notify_id:{$notify_id}"));//测试可以删除
            //获取支付宝远程服务器ATN结果（验证是否是支付宝发来的消息）
            $responseTxt = 'true';
            if (!empty($notify_id)) {
                $responseTxt = $this->getResponse($notify_id);
            }
            //D("test")->add(array('test'=>"responseTxt:".$responseTxt));//测试可以删除
            //生成签名结果
            $isSign = $this->getSignVeryfy($decrypt_post_para, $notify["sign"], false);
            //D("test")->add(array('test'=>"isSign:".$isSign));//测试可以删除
    ...
    protected function setInfo($notify) {
        // D("test")->add(array('test'=>"setInfonotify:".json_encode($notify)));//测试可以删除
        /*         * *****支付宝手机网页支付异步回调**** */
        ...
        if ($notify_data) {
            ...
            D("test")->add(array('test' => $out_trade_no)); //测试可以删除
        }
        //支付状态
