...
/**************************微信支付配置******************************************/
define('SSLCERT_PATH',dirname(__FILE__).'/WxPayPubHelper/cacert/apiclient_cert.pem');
define('SSLKEY_PATH',dirname(__FILE__).'/WxPayPubHelper/cacert/apiclient_cert.pem');
define('CURL_TIMEOUT',"30");
class Waptenpay extends \Common\Org\Pay\Pay {
	function __construct($alipay_config=array()){
	    $this->alipay_config=$alipay_config;
	    $this->check();
		//dump($this->alipay_config);exit();
		define('APPID',$this->alipay_config['APPID']);
...
/****js_api_call***/
	...
	if (!isset($_GET['code']))
	{
		...
		$url = $jsApi->createOauthUrlForCode($backUl);
		//echo $url;exit();
		Header("Location: $url"); 
	}else
	{
	...
	//设置统一支付接口参数
	//sign已填,商户无需重复填写
	$unifiedOrder->setParameter("openid",$openid);//微信号
